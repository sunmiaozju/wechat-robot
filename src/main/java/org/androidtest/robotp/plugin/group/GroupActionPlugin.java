...
public class GroupActionPlugin implements Serializable, IPlugin {
	...
	public boolean reply(Group group, BaseMsg msg) {
		...
		switch (getReplyType()) {
		case PIC:
			result = MessageTools.sendPicMsgByGroupNickName(group.getGroupName(),
					Config.dir_home + File.separator + getReplyContent());
			break;
		case TEXT:
			result = MessageTools.sendGroupMsgByNickName(getReplyContent(), group.getGroupName());
			// String senderNickName =
			// WechatTools.getMemberDisplayOrNickNameByGroupNickName(group.getGroupName(),
			// msg.getStatusNotifyUserName());
			// String text = "@" + senderNickName + " 于" + WeekHelper.getCurrentWeek() +
			// "打卡运动第" + " 1 次，本周已经达标。已经运动了 1 次";
			// result = MessageTools.sendGroupMsgByNickName(text, group.getGroupName());

			break;
		case MEDIA:
			result = MessageTools.sendGroupFileMsgByNickName(group.getGroupName(),
					Config.dir_home + File.separator + getReplyContent());
			break;
		default:
			result = MessageTools.sendGroupMsgByNickName("暂不支持该类型消息。请联系开发处理。", group.getGroupName());
			break;
		}
		return result;
	}
	...
	public String toString() {
		return "ActionPlugin{" + "matchWords='" + matchWords + '\'' + ", matchType=" + matchType + ", matchRule="
				+ matchRule + ", replyType=" + replyType + ", replyContent='" + replyContent + '\'' + '}';
	}
}