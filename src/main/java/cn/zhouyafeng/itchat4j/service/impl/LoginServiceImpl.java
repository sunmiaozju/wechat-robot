							...
							if (selector.equals("2")) {
								...
								if (msgObj != null) {
									try {
										...
										for (int j = 0; j < modContactList.size(); j++) {
											String groupUserName = modContactList.getJSONObject(j)
													.getString("UserName");
											String oldNickName = null;
											String groupNickName = modContactList.getJSONObject(j)
													.getString("NickName");
											boolean isExist = false;
											/**
											 * update grouplist
											 */
											List<JSONObject> groupList = Core.getInstance().getGroupList();
											for (int i = 0; i < groupList.size(); i++) {
												if (groupList.get(i).containsValue(groupUserName)) {
													isExist = true;
													oldNickName = groupList.get(i).getString("NickName");
													// LOG.info("已经从群列表删除:"
													// + groupList.get(i));
													groupList.remove(i);
											...
											// + modContactList
											// .getJSONObject(j));
											Core.getInstance().getGroupList().add(modContactList.getJSONObject(j));
											/**
											 * update groupid
											 */
											List<String> groupIdList = Core.getInstance().getGroupIdList();
											if (!groupIdList.contains(groupUserName)) {
												// LOG.info("已经添加进群ID列表:"
												// + groupUserName);
												Core.getInstance().getGroupIdList().add(groupUserName);
											} else {
												 ...
												 */
												for (int i = 0; i < Constant.groupList.size(); i++) {
													Group group = Constant.groupList.get(i);
													String groupId = group.getGroupId();
													if (groupId != null && groupId.equals(groupUserName)) {
											...
											List<String> groupNickNameList = Core.getInstance().getGroupIdList();
											if (isExist && oldNickName != null) {
												if (groupNickNameList.contains(oldNickName)) {
													groupNickNameList.remove(oldNickName);
													// LOG.info("已经从群NickName列表删除:"
													// + oldNickName);
												}
											}
											if (!groupNickNameList.contains(groupNickName)) {
												// LOG.info("已经添加进群NickName列表:"
												// + groupNickName);
												Core.getInstance().getGroupIdList().add(groupNickName);
											}
										}
									} catch (Exception e) {
										e.printStackTrace();
								...
								if (msgObj != null) {
									try {
										JSONArray msgList = new JSONArray();
										msgList = msgObj.getJSONArray("AddMsgList");
										msgList = msgObj.getJSONArray("ModContactList");
